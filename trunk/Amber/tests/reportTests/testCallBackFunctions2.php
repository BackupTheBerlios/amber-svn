...
// This test checks, if all userfunctions are called correctly in a report 
// - without data but with datasource
// - and canceling Report_NoData

class testCallBackFunctions2 extends AmberReport_UserFunctions
...
//TEST  
  function assertHtml($html)
  {
    ...
    $test->assertEquals(0, $this->calls['Report_ComputeColumns'], $id . ' Report_ComputeColumns');
    $test->assertEquals(0, $this->calls['Report_OnLoadData'], $id . ' Report_OnLoadData');
    $test->assertEquals(1, $this->calls['Report_NoData'], $id . ' Report_NoData');
    $test->assertEquals(0, $this->calls['Report_EvaluateExpressions'], $id . ' Report_EvaluateExpressions');
    $test->assertEquals(0, $this->calls['Report_OnNextRecord'], $id . ' Report_OnNextRecord');
    $test->assertEquals(0, $this->calls['Report_Page'], $id . ' Report_Page');
    $test->assertEquals(1, $this->calls['Report_Close'], $id . ' Report_Close');
    $test->assertEquals(0, $this->calls['Detailbereich_Format'], $id . ' Detailbereich_Format');
    $test->assertEquals(0, $this->calls['Detailbereich_Print'], $id . ' Detailbereich_Print');
    $test->assertEquals(0, $this->calls['Berichtskopf_Format'], $id . ' Berichtskopf_Format');
    $test->assertEquals(0, $this->calls['Berichtskopf_Print'], $id . ' Berichtskopf_Print');
    $test->assertEquals(0, $this->calls['Berichtsfuss_Format'], $id . ' Berichtsfuss_Format');
    $test->assertEquals(0, $this->calls['Berichtsfuss_Print'], $id . ' Berichtsfuss_Print');
    $test->assertEquals(0, $this->calls['Seitenkopfbereich_Format'], $id . ' Seitenkopfbereich_Format');
    $test->assertEquals(0, $this->calls['Seitenfussbereich_Print'], $id . ' Seitenfussbereich_Print');
    $test->assertEquals(0, $this->calls['Gruppenkopf0_Format'], $id . ' Gruppenkopf0_Format');
    $test->assertEquals(0, $this->calls['Gruppenkopf0_Print'], $id . ' Gruppenkopf0_Print');
    $test->assertEquals(0, $this->calls['Gruppenfuss1_Format'], $id . ' Gruppenfuss1_Format');
    $test->assertEquals(0, $this->calls['Gruppenfuss1_Print'], $id . ' Gruppenfuss1_Print');
    $test->assertEquals(0, $this->calls['Gruppenkopf2_Format'], $id . ' Gruppenkopf2_Format');
    $test->assertEquals(0, $this->calls['Gruppenkopf2_Print'], $id . ' Gruppenkopf2_Print');
    $test->assertEquals(0, $this->calls['Gruppenfuss3_Format'], $id . ' Gruppenfuss3_Format');
    $test->assertEquals(0, $this->calls['Gruppenfuss3_Print'], $id . ' Gruppenfuss3_Print');
  }
